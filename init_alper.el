...
;;; init_alper.el --- Initialization file for Emacs
...
;; C-u C-x C-x get back to latest jump point

;; smerge-next bound to smerge-command-prefix (n) to move to next conflict.
;; smerge-previous bound to smerge-command-prefix (p) to move to previous conflict.
...
;;
;; Search the files in the current project for a string. By default, this uses
;; the symbol at point. With a prefix argument, it will prompt for a regular
;; expression to search.

;; In your case, both M-< and pasting set a mark, so you want to go two marks back,
;; hence C-2 C-SPC or C-u C-SPC C-u C-SPC
;; Solve pdf focus, C-s compile tex automatically
...
;; https://stackoverflow.com/a/14838150/2402577 ------------------------
(require 'package)
...
;; swiper
(if (not (package-installed-p 'swiper))
    (progn
...
;; realgud
(if (not (package-installed-p 'realgud))
    (progn
...
(defun my-find-files ()
  ...
  (find-dired "~/eBlocBroker" "-name \\*.py ! -name flycheck_\\*.py ! -name __init__.py"))

(global-set-key (kbd "M-m") 'my-find-files)
...
(global-set-key "\C-x\C-r"  'query-replace-from-top)
...
(defun save-all ()
  (interactive)
  (my-save-all)
  (bk-kill-buffers "magit: eBlocBroker")
...
(custom-set-variables
 ...
 '(package-selected-packages
   (quote
    (selectrum helm-ag yasnippet-snippets languagetool org-journal eglot dumb-jump ag realgud powershell sound-wav company-dict go-mode pine-script-mode easy-kill company-quickhelp-terminal company-quickhelp exec-path-from-shell company-box shell-command shell-command+ el-get treemacs magit magit-delta markdown-mode helm-swoop shackle gcmh findr auto-indent-mode vi-tilde-fringe smex which-key-posframe goto-last-change ## fzf flycheck-pycheckers help-find-org-mode importmagic ipython-shell-send xclip simpleclip use-package-hydra yaml-mode dired-ranger ranger nswbuff helm-flyspell dotenv-mode pyenv-mode json-mode which-key auctex flymake-solidity company-solidity ace-window popwin helm rope-read-mode projectile hl-todo hippie-expand-slime iedit wgrep flycheck-mypy blacken evil solidity-flycheck solidity-mode use-package flycheck undo-tree)))
 '(select-enable-clipboard t)
 '(tab-stop-list nil)
 '(undo-tree-auto-save-history t)
 '(undo-tree-visualizer-diff nil)
 '(xref-after-jump-hook (quote (recenter)))
...
(global-set-key "\C-c\C-i" 'python-import-add)
(require 'uniquify)
...
(custom-set-faces
 ...
 '(ivy-current-match ((((class color) (background light)) (:background "#1a4b77" :foreground "white" :extend t)) (((class color) (background dark)) (:background "#44475a" :foreground "white" :extend t))))
 '(magit-diff-context-highlight ((t (:background "#464752" :foreground "white"))))
...
;; (add-hook 'mouse-leave-buffer-hook 'abort-recursive-edit)
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;

; https://emacs.stackexchange.com/a/24653
;  M-x flyspell-buffer
;; I assigned flyspell-lazy-check-pending to M-q, in addition to fill-paragraph
;; of course. That way, if I fill a paragraph, spelling is triggered
;; instantaneously, i.e. I don’t have to wait for flyspell-lazy. Also, I let
;; flyspell-buffer run when a text file is loaded.

; (define-key elpy-mode-map (kbd "C-c C-c") #'undefined)
;; (define-key elpy-mode-map (kbd "C-c C-p") #'undefined) ; TODO
;; (define-key elpy-mode-map (kbd "C-c C-b") #'break-point)
...
;; insers f"{}" and move cursor two char left
(defun _f ()
  (interactive)
  (insert "f\"{}\"")
  (backward-char)
...
;; (global-set-key "\M-o" 'hello-world)
...
;; TODO:  (goto-last-change 1)  emacsclient
;; ============================= ;; icicle-buffer
...
(defhydra python-indent (global-map "C-c")  ;; for global
...
(global-set-key (kbd "C-c o")
                (lambda () (interactive) (find-file "~/eBlocBroker/TODO")))
...
;; ;; --------------------------------------------------------------------
;; ;; doc: https://github.com/python-rope/ropemacs rope
;; ;; https://stackoverflow.com/a/2855895/2402577
;; ;; https://github.com/dgentry/Pymacs
;; ;; (setq elpy-rpc-python-command "python3")
;; (require 'pymacs)
;; (pymacs-load "ropemacs" "rope-")
;; (setq ropemacs-enable-autoimport 't)
;; ;; TODO: keybinding for rope-auto-import
...
(setq company-auto-complete t)
(global-set-key (kbd "C-c C-k") 'company-jedi)
...
(defun alper-end () (interactive)(message "Hello Alper"))
...
;;   '(cl-dolist (key '("C-c C-k"))
;;      (define-key elpy-mode-map (kbd key) nil)))

;; (eval-after-load "elpy"
;;   '(cl-dolist (key '("C-c C-d"))
;;      (define-key elpy-mode-map (kbd key) nil)))
